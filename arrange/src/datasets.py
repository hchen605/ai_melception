...
class MidiDataModule(pl.LightningDataModule):
	...
	def setup(self, stage=None):
		...
		#n_valid = int(self.train_val_test_split[1] * len(self.files))  # 12430
		#n_test = int(self.train_val_test_split[2] * len(self.files))  # 6215
...
class MidiDataset_Desc(IterableDataset):
	...
	def __iter__(self):
		worker_info = torch.utils.data.get_worker_info()
		self.split = _get_split(self.files, worker_info)  # self.files = midi file

		split_len = len(self.split)
		for i in range(split_len):
			try:
				current_file = self.load_file(self.split[i])  # includes Rhythm Intensity
			except ValueError as err:
				if self.print_errors:
					print(err)
				# raise err
			...
			events = current_file['events']  # includes Rhythm Intensity
			# Identify start of bars
			bars, bar_ids = self.get_bars(events, include_ids=True)
			if len(bars) > self.max_bars:
				if self.print_errors:
	...
	def load_file(self, file):
		...
		with open(file_path, 'r') as file:
			...
			content = file.read().split(",")  # includes Rhythm Intensity
			...
			sample = {
				'events': events,
				'description': description
			}  # both includes rhythm intensity
